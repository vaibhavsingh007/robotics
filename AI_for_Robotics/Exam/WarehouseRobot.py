...
def plan(warehouse, dropzone, todo):
    ...
    for t in todo:
        ...
        #while cells < t and r < rows:
        #    r += 1
        #    cells = r * cols
        #else:
        #    r -= 1  # For index this is one less
        #    c = (t - (r * cols)) - 1
        
        # Mark target as passable (PS: r,c must be valid)
        warehouse[r][c] = 0

        # Start search with [r,c] as target
        sRes = search(warehouse, dropzone, [r,c])
        if len(sRes) > 0:
            # Double the cost (for heading back to dropzone) and mark target as 0 (passable)
            tCost += sRes[0]*2;

    return tCost

# Modified to include diagonals
from copy import deepcopy
def search(warehouse,init,goal):
