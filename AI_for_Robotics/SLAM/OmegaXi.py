...
# This version includes With Landmarks
def doit(initial_pos, move1, move2, z0, z1, z2):
    pLen = 4    # No. of moves (or positions) + landmarks
    xi = [[0.0] for i in range(pLen)]
    omega = [[0.0]*pLen for i in range(pLen)]

    # This was a hint from the soln. for the answers didn't match.
    # The first cells are initialized to 1 and init resp.
    omega[0][0] = 1
    xi[0][0] = initial_pos

    params = [initial_pos, move1, move2]
    l1d = [z0, z1, z2]  # Respective landmark distances (-1 if not visible)
    ...
    for j in range(len(params) - 1):
        ...
        if (j+1) < len(params):
            ...
            x = positions[j]   # current location
            m = params[j+1]    # move
            y = x + m          # next location
