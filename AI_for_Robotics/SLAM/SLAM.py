...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    omega_x = matrix(omega_xi_x[0])
    xi_x = matrix(omega_xi_x[1])
    omega_y = matrix(omega_xi_y[0])
    xi_y = matrix(omega_xi_y[1])
...
def get_omega_xi(data, num_landmarks, axis, motion_noise, measurement_noise):
    indices = len(data) + num_landmarks
    ...
    for j in range(len(data)):
        ...
        for l in range(len(data[j][0])):
            ...
            cli = len(data) + cl[0] # Current landmark index
            cld = cl[axis+1]        # Current landmark distance from x


            omega[j][j] += measurementConfidence
            omega[cli][cli] += measurementConfidence
            omega[j][cli] -= measurementConfidence
            omega[cli][j] -= measurementConfidence
            
            xi[j][0] += -cld*measurementConfidence
            xi[cli][0] += cld*measurementConfidence

        if (j+1) < len(data):
            omega[j][j] += 1*motionConfidence
            omega[j+1][j+1] += 1*motionConfidence

            x = data[j][1][axis]        # current location
            m = data[j+1][1][axis]      # next location
            #m = y - x                   # move (delta)                   

            omega[j][j+1] -= motionConfidence
            omega[j+1][j] -= motionConfidence
        
            xi[j][0] += -m*motionConfidence
            xi[j+1][0] += m*motionConfidence
