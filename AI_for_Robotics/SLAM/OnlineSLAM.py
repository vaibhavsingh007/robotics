...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    # Adding one data element (at 0 index) as N-1 moves are generated by make_data fn
    data.append([[],[0,0]])

    omega_xi_x = get_omega_xi(data, num_landmarks, 0, motion_noise, measurement_noise)
    omega_xi_y = get_omega_xi(data, num_landmarks, 1, motion_noise, measurement_noise)

    omega_x = matrix(omega_xi_x[0].value)
    xi_x = matrix(omega_xi_x[1].value)
    omega_y = matrix(omega_xi_y[0].value)
    xi_y = matrix(omega_xi_y[1].value)

    mu_x = omega_x.inverse() * xi_x
    mu_y = omega_y.inverse() * xi_y

    mu = []     #[[0] for i in range(len(mu_x.value))]

    for i in range(len(mu_x.value)):
        #mu[i] = [mu_x.value[i][0], mu_y.value[i][0]]
        mu.append([mu_x.value[i][0]])
        mu.append([mu_y.value[i][0]])

    # Combine Omega x and y
    indices = num_landmarks+1
    omega = [[0]*(2*(indices)) for i in range(2*(indices))]

    for i in range(0,(2*indices),2):
        for j in range(0,(2*indices),2):
            omega[i][j] = omega[i+1][j+1] = omega_x.value[i/2][j/2]
            omega[i][j+1] = omega[j+1][i] = 0.0

    return [matrix(mu), matrix(omega)] # Make sure you return mu for grading!

def get_omega_xi(data, num_landmarks, axis, motion_noise, measurement_noise):
    indices = 1 + num_landmarks
    omega = [[0]*indices for i in range(indices)]
    xi = [[0] for i in range(indices)]
    ...
    motionConfidence = 1./motion_noise
    measurementConfidence = 1./measurement_noise

    expand_list = [i for i in range(indices+1) if i != 1]
    take_list = [i for i in range(1,indices+1)]

    for j in range(len(data)):
        # And now is the time for landmark updates
        for l in range(len(data[j][0])):
            cl = data[j][0][l]      # Current landmark
            cli = 1 + cl[0] # Current landmark index
            cld = cl[axis+1]        # Current landmark distance from x


            omega.value[0][0] += measurementConfidence
            omega.value[cli][cli] += measurementConfidence
            omega.value[0][cli] -= measurementConfidence
            omega.value[cli][0] -= measurementConfidence
            
            xi.value[0][0] += -cld*measurementConfidence
            xi.value[cli][0] += cld*measurementConfidence

        # Motion updates
        if (j+1) < len(data):
            # Expand omega and xi at index 1 (both row and column)
            omega = omega.expand(indices+1, indices+1, expand_list)
            xi = xi.expand(indices+1, 1, expand_list, [0])

            omega.value[0][0] += 1*motionConfidence
            omega.value[1][1] += 1*motionConfidence

            m = data[j][1][axis]

            omega.value[0][1] -= motionConfidence
            omega.value[1][0] -= motionConfidence
        
            xi.value[0][0] += -m*motionConfidence
            xi.value[1][0] += m*motionConfidence

            # Take omega and xi primes (subsets) and scavenge
            A = omega.take([0], take_list)
            B = omega.take([0],[0])
            C = xi.take([0],[0])
            omegaPrime = omega.take(take_list)
            xiPrime = xi.take(take_list, [0])
