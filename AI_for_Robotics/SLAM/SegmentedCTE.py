...
class plan:
    ...
    def make_heuristic(self, grid, goal, cost):
        self.heuristic = [[0]*len(grid[0]) for i in range(len(grid))]
    ...
    def astar(self):
        ...
        closed = [[0]*len(self.grid[0]) for row in range(len(self.grid))] 
        action = [[0]*len(self.grid[0]) for row in range(len(self.grid))] 
...
class robot:
    ...
    # --------
    # check: 
    #    checks if the robot collides with an obstacle, or
    ...
    # --------
    # measurement_prob
    #   computes the probability of a measurement, viz. P that
    #   current robot instance (particle) is the one closest to
    #   current measurement.
...
def run(grid, goal, spath, params, printflag = False, speed = 0.1, timeout = 1000):
    ...
    while not myrobot.check_goal(goal) and N < timeout:
        ...
        # Check this method. It is basically the mean of all particles.
        estimate = filter.get_position()

        ### ENTER CODE HERE
        
        current_segment_x1 = spath[index][0]
        current_segment_y1 = spath[index][1]
        current_segment_x2 = spath[index + 1][0]
        current_segment_y2 = spath[index + 1][1]

        # Refer to the reference image for more info on following calc. 
        rx = myrobot.x - current_segment_x1     # robot's delta x wrt. pt. 1
        ry = myrobot.y - current_segment_y1     # robot's delta y wrt. pt. 1
        delta_x = current_segment_x2 - current_segment_x1
        delta_y = current_segment_y2 - current_segment_y1

        u = (rx*delta_x + ry*delta_y)/(delta_x**2 + delta_y**2)

        # Calculate CTE using current segment.
        cte = (ry*delta_x - rx*delta_y)/(delta_x**2 + delta_y**2)

        # Check if the robot has not crossed past the current (line) segment. 
