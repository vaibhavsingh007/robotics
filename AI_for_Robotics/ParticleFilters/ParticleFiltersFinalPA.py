...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        if (len(motion) != 2):
            raise ValueError, "Motion vector must contain two values."
        
        steering_angle = motion[0]
        d = motion[1]

        # Update steering angle and distance using Gaussian (noise)
        steering_angle = random.gauss(steering_angle, self.steering_noise)
        d = random.gauss(d, self.distance_noise)

        if (steering_angle > max_steering_angle):
            raise ValueError, "Steering angle must be less than %s" % max_steering_angle

        beta = (d/self.length) * tan(steering_angle)  # Turning angle
        
        if (beta < 0.001):  # Special case (straight motion)
            new_x = self.x + d*cos(self.orientation)
            new_y = self.y + d*sin(self.orientation)
            new_orientation = (self.orientation + beta) % (2*pi)
        else:
            radius = d/beta  # Turning radius

            # Calculate centre co-ordinates
            cx = self.x - sin(self.orientation)*radius
            cy = self.y + cos(self.orientation)*radius

            # Calculate new robot location and orientation
            new_orientation = (self.orientation + beta) % (2*pi)
    ...
    def sense(self, add_noise = 1): #do not change the name of this function
        ...
        for landmark in landmarks:
            ...
            # Don't forget to normalize bearing/angle
            bearing %= (2*pi)
...
def particle_filter(motions, measurements, N=500): # I know it's tempting, but don't change N!
    ...
    return get_position(p)  # Get the mean position from the resampled particles
...
## Real measurements for each move defined in motions above
